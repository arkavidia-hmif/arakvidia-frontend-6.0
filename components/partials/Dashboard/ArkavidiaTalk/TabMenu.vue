    ...
    <v-card elevation="3">
      <div class="pa-4">
        ...
        <div class="grey--text text--darken-2">
          {{ registrant.mainevent.shortDesc }}
        </div>
      </div>
      <v-divider />
      <v-list class="pt-0">
        <v-list-item v-if="registrant.mainevent.isRegistrationOpen" @click="dialog = true">
        ...
        <template v-for="(stage, i) in stages">
          <v-list-item :key="i" :to="stage.id === registrant.activeStageId ? `/dashboard/arkavidia-talk/${slug}` : undefined" exact :disabled="stage.id !== registrant.activeStageId">
            <v-list-item-title>
              {{ stage.name }}
            </v-list-item-title>
          </v-list-item>
...
export default class TabMenu extends Vue {
    ...
    deleteRegistration() {
      ...
      this.deleteRegistrantAction({ registrantId: this.registrant.id })
        .then(() => {
          const redirectUrl = '/dashboard/arkavidia-talk';
